...
class CriresSpecphotStdReport(CriresReportMixin, MasterSpecphotStdReport):
    ...
    def parse_sof(self):
        ...
        for filename, catg in self.inputs:
            if catg == "CAL_NODDING_OTHER" or catg == "CAL_NODDING_JITTER":
                raw_flat = filename

            if catg == "OBS_NODDING_COMBINEDA":
                combine_a = filename

            elif catg == "OBS_NODDING_COMBINEDB":
                combine_b = filename

            elif catg == "OBS_NODDING_EXTRACT_COMB":
                extract_comb = filename

        file_lists = []
        if (
            combine_a is not None
            and combine_b is not None
            and extract_comb is not None
            and raw_flat is not None
        ):
            file_lists.append(
                {
                    "combine_a": combine_a,
    ...
    def generate_panels(self, **kwargs):
        ...
        input_files = [combine_a.filename(),
                       combine_b.filename(),
                       ext_comb.filename(),
                       raw_flat.filename()
                      ]
        for ext in self.data_extensions:
            ...
            for i in range(len(spec_arr)):
                ...
                if np.all(np.isnan(spec)):
                    ...
                    line_plot = self.plot_vs_wavelength(
                        data=spec, label=spec_arr[i], title="OBS_NODDING_EXTRACT_COMB"
                    )
            ...
            description = (
                f"Specphotometric Std panel - (extension {ext})"
            )
            panels[p] = {
